...
exports.workflowToToolName = workflowToToolName;
exports.buildToolDescription = buildToolDescription;
...
function workflowToToolName(name) {
    return name
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '_')
        .replace(/^_|_$/g, '')
...
function buildToolDescription(workflow) {
    const tags = workflow.tags?.map(t => t.name).join(', ') || '';
    const nodeTypes = [...new Set(workflow.nodes.map(n => n.type.split('.').pop()))].join(', ');
    return [
