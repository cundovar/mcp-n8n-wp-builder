...
let lastWorkflowFetchError: string | null = null;
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    if (name === 'n8n_create_workflow') {
      const workflow = await createWorkflow(args?.workflow as any);
    ...
    if (name === 'n8n_delete_workflow') {
      const result = await deleteWorkflow(args?.workflow_id as string);
      invalidateWorkflowCache();
      return {
        content: [{
          type: 'text',
    ...
    if (name === 'n8n_set_workflow_active') {
      const result = await setWorkflowActive(
        args?.workflow_id as string,
        Boolean(args?.active)
      );
      invalidateWorkflowCache();
